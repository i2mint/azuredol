...
from dol import KvReader, KvPersister, filt_iter, wrap_kvs, ValueCodecs

# TODO: Implement three types of blobs: block, append, and page blobs (see https://github.com/i2mint/azuredol/discussions/2#discussioncomment-12176104)
# TODO: Use cache_property to cache the container client
# TODO: Implement appendable blob objects
# TODO: Implement close to metal len and contains methods
# TODO: Do we really need/want the key codecs in the base class?
...
class AzureReader(KvReader):
    ...
    def __init__(
        ...
        *,
        ...
        self.path = f"{path.strip('/')}/" if path else ''
        self.create_container_if_missing = create_container_if_missing

    def __repr__(self):
        return f'{self.__class__.__name__}({self.container_name!r}, path={self.path!r})'
    ...
    def __getitem__(self, k):
        ...
        if _id.endswith('/'):
            _new_init_kwargs = dict(self._init_kwargs, path=_id)
            return self.__class__(**_new_init_kwargs)
        blob_client = self._container_client.get_blob_client(blob=_id)
...
class AzureFiles(AzureReader, KvPersister):
    """
...
# TODO: Replace decoder by pickle-able one
AzureTextFiles = wrap_kvs(AzureFiles, value_decoder=lambda x: x.decode())
